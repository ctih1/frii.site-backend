...
class TestDomainUser:
    ...
    def test_domain_not_free(self):
        assert not Validation(domain_table, dns).is_free(
            "testing-domains", "A", {}, False
        )

    def test_valid_subdomain(self):
        assert Validation(domain_table, dns).is_free(
            "example.testing-domains", "A", pytest.example_user["domains"]
        )
