...
DatabaseKeyFormat = TypedDict("DatabaseKeyFormat", {"val": str, "contributor": str})

DatabaseLanguageFormat = TypedDict(
    "DatabaseLanguageFormat", {"_id": str, "keys": Dict[str, DatabaseKeyFormat]}
)
...
class Translations(Table):
    def __init__(self, mongo_client: MongoClient):
        ...
        for file in response.json():
            thread: threading.Thread = threading.Thread(
                target=self.__init_language, args=(file,)
            )
    ...
    def __process_missing_keys(self, language: str, keys: dict) -> None:
        ...
        for key in self.main_language:
            ...
            if key not in self.languages[language] and key not in preview_keys.get(
                "keys", {}
            ):
                self.missing_keys[language]["misses"] += 1
                self.missing_keys[language]["keys"].append(
                    {"key": key, "ref": self.main_language.get(key)}
                )
    ...
    def __calculate_percentages(self, use_int: bool = False) -> Dict[str, float | int]:
        ...
        for language in self.languages:
            thread = threading.Thread(
                target=self.__process_missing_keys,
                args=(
                    language,
    ...
    def add(self, lang: str, keys: List[Dict[Any, Any]], username: str):
        ...
        for translation in keys:
            if translation["val"] != "":
                try:
                    self.keys[lang].pop(
                        list(self.keys[lang]).index(
                            {
                                "key": translation["key"],
                                "ref": self.languages["en"].get(translation["key"]),
                            }
                        )
                    )
                except ValueError:
                    logger.warning(
                        f"Faced a value error trying to delete key {translation['key']}"
                    )
        ...
        self.table.update_one({"_id": lang}, {"$set": language}, upsert=True)
